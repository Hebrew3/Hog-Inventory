...
const processGetAPI = async (req, res) => {
    ...
    switch (module) {
        case 'feed':
            switch (action) {
                case 'get-feed': {
                    let responseData = await feedModule.getFeed(dbHelper, data.feedId);
                    return res.status(responseData.status).json(responseData);
                }
                case 'get-all-feeds': {
                    let responseData = await feedModule.getFeeds(dbHelper, req.session, data.feedId);
                    return res.status(responseData.status).json(responseData);
                }
                default:
        ...
        case 'vaccine':
            switch (action) {
                case 'get-vaccine': {
                    let responseData = await vaccineModule.getVaccine(dbHelper, data.vaccineId);
                    return res.status(responseData.status).json(responseData);
                }
                case 'get-all-vaccines': {
                    let responseData = await vaccineModule.getVaccines(dbHelper, req.session, data.vaccineId);
                    return res.status(responseData.status).json(responseData);
                }
                default:
                    return res.status(404).json({ error: 'Unknown action' });
...
const processPostAPI = async (req, res) => {
    ...
    switch (module) {
        ...
        case 'feed':
            switch (action) {
                case 'create-feed': {
                    let responseData = await feedModule.createFeed(dbHelper, req.session, data);
                    return res.status(responseData.status).json(responseData);
                }
                case 'update-feed': {
                    let responseData = await feedModule.updateFeed(dbHelper, req.session, data);
                    return res.status(responseData.status).json(responseData);
                }
                case 'delete-feed': {
                    let responseData = await feedModule.deleteFeed(dbHelper, req.session, data.feedId);
                    return res.status(responseData.status).json(responseData);
                }
                default:
                    return res.status(404).json({ error: 'Unknown action' });
        ...
        case 'vaccine':
            switch (action) {
                case 'create-vaccine': {
                    let responseData = await vaccineModule.createVaccine(dbHelper, req.session, data);
                    return res.status(responseData.status).json(responseData);
                }
                case 'update-vaccine': {
                    let responseData = await vaccineModule.updateVaccine(dbHelper, req.session, data);
                    return res.status(responseData.status).json(responseData);
                }
                case 'delete-vaccine': {
                    let responseData = await vaccineModule.deleteVaccine(dbHelper, req.session, data.vaccineId);
                    return res.status(responseData.status).json(responseData);
                }
                default:
                    return res.status(404).json({ error: 'Unknown action' });
